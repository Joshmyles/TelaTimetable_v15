...
export class TimetableExcelDirective implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.uploadWorker = new Worker(
      new URL('../../src/app/upload-timetable.worker.ts', import.meta.url)
    );
  }
  ...
  convertWorkSheetToTimetable(
    workBook: XLSX.WorkBook,
    workSheetNames: string[]
  ): TimeTableExcel[] {
    return workSheetNames.map((workSheetName) => {
      const sheet: XLSX.WorkSheet = workBook.Sheets[workSheetName];
      let datas: TimeTableRowExcel[] =
        XLSX.utils.sheet_to_json<TimeTableRowExcel>(sheet, {
          defval: '',
          blankrows: false,
        });
      const timetable: TimeTableExcel = {
        schoolClass: { id: '123', name: workSheetName },
        rows: datas,
      };
