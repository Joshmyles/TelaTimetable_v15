...
export class SchoolFilterComponent implements OnInit {
  ...
  @Output() SelectedSchoolDetailEvent: EventEmitter<FilteredSchoolDetails> =
    new EventEmitter();
  ...
  ngOnInit() {
    ...
    this.schoolLevelControl.valueChanges
      .pipe(
        switchMap((value) => {
          //  console.log(this.localGovernmentControl.value)
          this.filteredSchoolDetails = {
            ...this.filteredSchoolDetails,
            schoolLevel: value,
          };
      ...
      map((value: string | Region) => {
        if (value === '') {
          ...
          if (typeof value === 'string') {
            return this.regionList.filter((lg) =>
    ...
    this.selectedRegion$
      .pipe(
        ...
        switchMap((region: Region) => {
          ...
          return this.schoolFilterService.getDistricts(
            new HttpParams().set('region', region.id)
          );
        })
      )
      .subscribe({
    ...
    this.filteredLocalGovernments$ =
      this.localGovernmentControl.valueChanges.pipe(
        startWith(''),
        map((value: string | District) => {
          if (value === '') {
            ...
            if (typeof value === 'string') {
              return this.localGovernmentList.filter((lg) =>
                lg.name.toLowerCase().includes(value)
              );
            } else {
    ...
    this.filteredSchools$ = this.schoolControl.valueChanges.pipe(
      ...
      map((value: string | School) => {
        if (value === '') {
          ...
          if (typeof value === 'string') {
            return this.schoolList.filter((lg) =>
              lg.name.toLowerCase().includes(value)
            );
          } else {
    ...
    this.selectedSchool$
      .pipe(
        ...
        switchMap((school: School) => {
          const params = new HttpParams().set('school', school.id);
          //.set("schoolLevel", this.schoolLevelControl.value)
        ...
        switchMap((term) => {
          ...
          if (this.schoolControl.value) {
            let params = new HttpParams()
              .set('academicTerm', term.id)
              .set('school', this.schoolControl.value.id);
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response) {
            this.filteredSchoolDetails = {
              ...this.filteredSchoolDetails,
              schoolClasses: response?.data.sort(this.sortFunction) || [],
            };
            this.SelectedSchoolDetailEvent.next(this.filteredSchoolDetails);
          }
        },
      ...
      next: (res) => {
        ...
        if (res.status) {
          ...
          const subs = res.data.filter(
            (sub) => sub.subjectClassification != null
          );
  ...
  sortFunction(
    a: Region | District | School,
    b: Region | District | School,
