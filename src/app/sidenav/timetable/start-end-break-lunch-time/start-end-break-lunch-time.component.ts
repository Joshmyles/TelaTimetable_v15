...
export class StartEndBreakLunchTimeComponent implements OnInit {
  ...
  @Output()
  classStartEndBreakLunchTimeData: EventEmitter<ClassStartEndBreakLunchTime> =
    new EventEmitter<ClassStartEndBreakLunchTime>();
  ...
  ngOnInit(): void {
    ...
    if (this.selectedLunchTimes.length > 0) {
      ...
      this.formGroup
        .get('lunchStartTime')
        ?.patchValue(startTime.startTime.format(TelaTimetablePattern));
      this.formGroup
        .get('lunchEndTime')
        ?.patchValue(endTime.endTime.format(TelaTimetablePattern));
    }
  }
