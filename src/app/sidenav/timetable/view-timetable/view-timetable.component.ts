...
export class ViewTimetableComponent implements OnInit {
  ...
  filterByClassName(
    dbTimetableLessons: DbTimetableLesson[],
    schoolClassName: string
  ): DbTimetableLesson[] {
    return dbTimetableLessons.filter(
      (dbTimetableLesson) =>
        dbTimetableLesson.schoolClass.name == schoolClassName
    );
  }
  ...
  filterClassTimes(
    ...
    return chain(dbTimetableLessons)
      .filter(
        (dbTimetableLesson) =>
          dbTimetableLesson.schoolClass.name.toLowerCase() ==
          schoolClassName.toLowerCase()
      )
      .uniqBy((dbTimetableLesson) => dbTimetableLesson.startTime)
      .map((dbTimetableLesson) => ({
        startTime: dbTimetableLesson.startTime,
        endTime: dbTimetableLesson.endTime,
      }))
      .value();
  }
  ...
  filterByStarTime(
    ...
    return dbTimetableLessons.filter(
      (dbTimetableLesson) => dbTimetableLesson.startTime == startTime
    );
  }
  ...
  findByWeekDay(
    dbTimetableLessons: DbTimetableLesson[],
    dayOfWeek: DayOfWeek
  ): DbTimetableLesson | undefined {
    return dbTimetableLessons.find(
      (dbTimetableLesson) =>
        dbTimetableLesson.lessonDay.toLocaleUpperCase() ==
        dayOfWeek.name().toLocaleUpperCase()
    );
  }

  // get day lesson from time list
  getLessonByDayOfWeek(
    dbTimetableLessons: DbTimetableLesson[],
    dayOfWeek: DayOfWeek
  ): DbTimetableLesson | undefined {
    return dbTimetableLessons.find(
  ...
  loadTimetable() {
    let params = new HttpParams()
      .set('school', this.filteredSchoolDetails?.school?.id as string)
      .set('academicTerm', this.filteredSchoolDetails?.term?.id as string);
    this.isLoading = true;
    const _$: Subscription = this.schoolFilterService
      .loadTimetable(params)
      .subscribe({
        next: (response) => {
        ...
        error: (error: HttpErrorResponse) => {
          ...
          if (error.error.error.errorCode == 404) {
            this.toastr.warning(
              `Timetable of ${this.filteredSchoolDetails?.school?.name} for ${this.filteredSchoolDetails?.term?.term} Not Found`
            );
  ...
  updateClassTimetable(timetableClass: string) {
    ...
    this.isEdit = false;
    const classLessons: DbTimetableLesson[] = this.dbTimetable.lessons.filter(
      (tl) => tl.schoolClass.name == timetableClass
    );
    ...
    const params = new HttpParams()
      .set('timetable', this.dbTimetable.id)
      .set('schoolClass', classLessons[0].schoolClass.id);

    const _$: Subscription = this.schoolFilterService
      .updateSchoolClassTimetable(params, classLessons)
      .subscribe({
        next: (_response) => {
          // console.log(response)
          this.isUpdating = false;
          this.toastr.success(
            `${classLessons[0].schoolClass.name} Timetable updated successfully`
          );
        },
        error: (error: HttpErrorResponse) => {
          this.isUpdating = false;
