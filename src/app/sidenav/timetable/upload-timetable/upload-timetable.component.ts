...
import {
  TimeTableExcel,
  TimeTableRowExcel,
} from 'src/shared/timetable-uploader.directive';
...
export class UploadTimetableComponent implements OnInit {
  ...
  getStaffByFullName(
    schoolStaffs: SchoolStaffWithSchool_DistrictDto[],
    fullName: string
  ): SchoolStaffWithSchool_DistrictDto | undefined {
    ...
    const selectedStaff: SchoolStaffWithSchool_DistrictDto | undefined =
      schoolStaffs.find(
        (staff) =>
          staff.firstName.toLocaleLowerCase() ===
            firstName.toLocaleLowerCase() &&
          staff.lastName.toLocaleLowerCase() === lastName.toLocaleLowerCase()
      );
  ...
  getSubjectByCode(
    ...
    return subjects.find(
      (sub) => sub.code.toLocaleLowerCase() == code.toLocaleLowerCase()
    );
  }

  getSchoolClassByName(
    schoolClasses: SchoolClass[],
    name: string
  ): SchoolClass | undefined {
    return schoolClasses.find(
      (schoolClasse) =>
        schoolClasse.name.toLocaleLowerCase() === name.toLocaleLowerCase()
    );
  }

  convertExcelTimetableToTimetable(
    timeTableExcels: TimeTableExcel[]
  ): SchoolTimeTable | null {
    const lessons: SchoolTimeTableLesson[] = timeTableExcels.flatMap(
      (timeTableExcel) => {
        const schoolClass = this.getSchoolClassByName(
          this.dbSchoolClass,
          timeTableExcel.schoolClass.name
        ) as SchoolClass;
        ...
        return timeTableExcel.rows.flatMap((row) => {
          ...
          const startTime = times[0].includes('am')
            ? times[0].replace('am', '').trim()
            : times[0].replace('pm', '').trim();
          const endTime = times[1].includes('am')
            ? times[1].replace('am', '').trim()
            : times[1].replace('pm', '').trim();

          const mon: string[] = row.MONDAY.split(':');
          const monSubject: SchoolSubject = this.getSubjectByCode(
            this.filteredSchoolDetails?.subjects || [],
            mon[0]
          ) as SchoolSubject;
          const monStaff: SchoolStaffWithSchool_DistrictDto =
            this.getStaffByFullName(
              this.filteredSchoolDetails?.staffList || [],
              mon[1]
            ) as SchoolStaffWithSchool_DistrictDto;
          ttLessons.push({
            id: '',
            lessonDay: 'monday',
            schoolClass,
            ...
            startTime,
            endTime,
            schoolStaff: monStaff,
          });

          const tue: string[] = row.TUESDAY.split(':');
          const tueSubject: SchoolSubject = this.getSubjectByCode(
            this.filteredSchoolDetails?.subjects || [],
            tue[0]
          ) as SchoolSubject;
          const tueStaff: SchoolStaffWithSchool_DistrictDto =
            this.getStaffByFullName(
              this.filteredSchoolDetails?.staffList || [],
              tue[1]
            ) as SchoolStaffWithSchool_DistrictDto;
          ttLessons.push({
            ...
            startTime,
            endTime,
            schoolStaff: tueStaff,
          });

          const wed: string[] = row.WEDNESDAY.split(':');
          const wedSubject: SchoolSubject = this.getSubjectByCode(
            this.filteredSchoolDetails?.subjects || [],
            wed[0]
          ) as SchoolSubject;
          const wedStaff: SchoolStaffWithSchool_DistrictDto =
            this.getStaffByFullName(
              this.filteredSchoolDetails?.staffList || [],
              wed[1]
            ) as SchoolStaffWithSchool_DistrictDto;
          ttLessons.push({
            ...
            startTime,
            endTime,
            schoolStaff: wedStaff,
          });

          const thurs: string[] = row.THURSDAY.split(':');
          const thursSubject: SchoolSubject = this.getSubjectByCode(
            this.filteredSchoolDetails?.subjects || [],
            thurs[0]
          ) as SchoolSubject;
          const thursStaff: SchoolStaffWithSchool_DistrictDto =
            this.getStaffByFullName(
              this.filteredSchoolDetails?.staffList || [],
              thurs[1]
            ) as SchoolStaffWithSchool_DistrictDto;
          ttLessons.push({
            ...
            lessonDay: 'thursday',
            schoolClass,
            subject: thursSubject,
            startTime,
            endTime,
            schoolStaff: thursStaff,
          });

          const fri: string[] = row.FRIDAY.split(':');
          const friSubject: SchoolSubject = this.getSubjectByCode(
            this.filteredSchoolDetails?.subjects || [],
            fri[0]
          ) as SchoolSubject;
          const friStaff: SchoolStaffWithSchool_DistrictDto =
            this.getStaffByFullName(
              this.filteredSchoolDetails?.staffList || [],
              fri[1]
            ) as SchoolStaffWithSchool_DistrictDto;
  ...
  showStaffList() {
    if (this.filteredSchoolDetails?.staffList) {
      ...
      ${this.filteredSchoolDetails?.staffList
        .map(
          (staff, index) => `${index + 1} ${staff.firstName} ${staff.lastName}`
        )
        .join('\n')}`);

      console.log(
        this.filteredSchoolDetails?.staffList.map(
          (staff, index) => `${index + 1} ${staff.firstName} ${staff.lastName}`
        )
      );
    }
  ...
  showSchoolClasses() {
    if (this.filteredSchoolDetails?.schoolClasses) {
      this.toastr.info(`${this.filteredSchoolDetails?.school?.name} staff list
      ${this.filteredSchoolDetails?.schoolClasses
        .map((scchoolClass, index) => `${index + 1} ${scchoolClass.name}`)
        .join('\n')}`);
    }
  }
  ...
  onSubjectChange(
    ...
    switch (day) {
      ...
      case DayOfWeek.FRIDAY:
        ...
        break;
      default:
        console.log('Invalid DAY ', day.name());
    }
  ...
  saveTimetable() {
    ...
    const schoolTimetable: SchoolTimeTable | null =
      this.convertExcelTimetableToTimetable(this.timeTableExcels);

    if (schoolTimetable) {
      const someInvalid: boolean = schoolTimetable.timeTableLessons.some(
        (lesson) =>
          lesson.schoolStaff == undefined || lesson.subject == undefined
      );
      if (someInvalid) {
        const invalidLessons: SchoolTimeTableLesson[] =
          schoolTimetable.timeTableLessons.filter(
            (lesson) =>
              lesson.schoolStaff == undefined || lesson.subject == undefined
          );
        const invalidByClass: Dictionary<SchoolTimeTableLesson[]> = groupBy(
          invalidLessons,
          (lesson) => lesson.schoolClass.name
        );

        // console.log("invalidByClass " , invalidByClass)

        this.invalidLessonCounts = Object.keys(invalidByClass).map(
          (className) => ({
            name: className,
            count: invalidByClass[className].length,
          })
        );
        ...
        const lessons: SchoolTimeTableLesson[] =
          schoolTimetable?.timeTableLessons.map((l) => {
            const schoolClass: SchoolClass | undefined =
              this.filteredSchoolDetails?.schoolClasses.find(
                (sl) => sl.name === l.schoolClass.name
              );
            if (schoolClass) {
        ...
        schoolTimetable.academicTerm = this.filteredSchoolDetails
          ?.term as AcademicTerm;
  ...
  uploadToServer(schoolTimetable: SchoolTimeTable) {
    ...
    const _$: Subscription = this.schoolFilterService
      .uploadTimetable(schoolTimetable)
      .subscribe({
        next: (response) => {
        ...
        error: (error: HttpErrorResponse) => {
          ...
          if (error.error.error.errorCode == 409) {
            this.toastr.warning(
              `Timetable of ${this.filteredSchoolDetails?.school?.name} for ${this.filteredSchoolDetails?.term?.term} Already exists  , for changes update`
            );
